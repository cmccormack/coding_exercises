...
function checkLeft(arr, current) {
  return checkRight(arr.reverse(), current);
}
function checkRight(arr, current) {
  let count = 0;
...
function countSubarrays(arr) {
  return arr.map(
    (v, i) =>
      checkLeft(arr.slice(0, i + 1), v) + checkRight(arr.slice(i), v) + 1
  );
}
